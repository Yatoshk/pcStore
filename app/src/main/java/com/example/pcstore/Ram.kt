...
import com.example.pcstore.databinding.RamActivityBinding

class Ram : AppCompatActivity() {
    lateinit var binding: RamActivityBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = RamActivityBinding.inflate(layoutInflater)

        var db: DataBase = DataBase.getDatabaseRam(this)


        val dao = db.getDao()

        db.getDao().getAllItemRam().asLiveData().observe(this) {
            val grid = findViewById<GridLayout>(R.id.grid_ram)
    ...
    private fun createListDynamically(row: Int, grid: GridLayout, dao: ItemsDao, it: RamItem) {
        ...
        pic.setImageResource(R.drawable.ram2)
        ...
        mark.setOnClickListener {
            when (favoritesState) {
                true -> {
                    ...
                    Thread {
                        dao.updateFavoritesRamById(false, id!!)
                    }.start()
                }
                ...
                false -> {
                    ...
                    Thread {
                        dao.updateFavoritesRamById(true, id!!)
                    }.start()
                }
            }
        }
        ...
        comparison.setOnClickListener {
            when (comparisonState) {
                true -> {
                    ...
                    Thread {
                        dao.updateComparisonRamById(false, id!!)
                    }.start()
                }
                ...
                false -> {
                    ...
                    Thread {
                        dao.updateComparisonRamById(true, id!!)
                    }.start()
                }
            }
        }
        ...
        busket.setOnClickListener {
            when (bucketState) {
                ...
                false -> {
                    ...
                    Thread {
                        dao.updateBucketRamById(true, id!!)
                    }.start()
                }
            }
        }
