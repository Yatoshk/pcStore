...
import com.example.pcstore.databinding.CoolingActivityBinding

class Cooling: AppCompatActivity() {
    lateinit var binding: CoolingActivityBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = CoolingActivityBinding.inflate(layoutInflater)

        var db: DataBase = DataBase.getDatabaseCooling(this)


        val dao = db.getDao()

        db.getDao().getAllItemCooling().asLiveData().observe(this) {
            val grid = findViewById<GridLayout>(R.id.grid_cooling)
    ...
    private fun createListDynamically(row: Int, grid: GridLayout, dao: ItemsDao, it: CoolingItem) {
        ...
        val typeC = it.typeC
        val fans = it.fans
        val heatGeneration = it.heatGeneration
        ...
        mark.setOnClickListener {
            when (favoritesState) {
                true -> {
                    ...
                    Thread {
                        dao.updateFavoritesCoolingById(false, id!!)
                    }.start()
                }
                ...
                false -> {
                    ...
                    Thread {
                        dao.updateFavoritesCoolingById(true, id!!)
                    }.start()
                }
            }
        }
        ...
        comparison.setOnClickListener {
            when (comparisonState) {
                true -> {
                    ...
                    Thread {
                        dao.updateComparisonCoolingById(false, id!!)
                    }.start()
                }
                ...
                false -> {
                    ...
                    Thread {
                        dao.updateComparisonCoolingById(true, id!!)
                    }.start()
                }
            }
        }
        ...
        busket.setOnClickListener {
            when (bucketState) {
                ...
                false -> {
                    ...
                    Thread {
                        dao.updateBucketCoolingById(true, id!!)
                    }.start()
                }
            }
        }
