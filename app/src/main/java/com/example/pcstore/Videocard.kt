...
import com.example.pcstore.databinding.VideocardActivityBinding

class Videocard: AppCompatActivity() {
    lateinit var binding: VideocardActivityBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = VideocardActivityBinding.inflate(layoutInflater)

        var db: DataBase = DataBase.getDatabaseVideocard(this)


        val dao = db.getDao()

        db.getDao().getAllItemVideocard().asLiveData().observe(this) {
            val grid = findViewById<GridLayout>(R.id.grid_videocard)
    ...
    private fun createListDynamically(row: Int, grid: GridLayout, dao: ItemsDao, it: VideocardItem) {
        ...
        pic.setImageResource(R.drawable.videocard2)
        pic.setPadding(50, 15, 0 ,0)
        pic.layoutParams = paramTableRow

        val infoText = TextView(this)
        infoText.layoutParams = paramTableRow
        infoText.text = "Тип памяти: ${memory}\n\nОбъём памяти: ${capacity}\n\nЧастота памяти: ${frequency} МГц"
        ...
        mark.setOnClickListener{
            when (favoritesState) {
                true -> {
                    ...
                    Thread{
                        dao.updateFavoritesVideocardById(false, id!!)
                    }.start()
                }
                false -> {
                    mark.setImageResource(R.drawable.bookmark_active)
                    Thread{
                        dao.updateFavoritesVideocardById(true, id!!)
                    }.start()
                }
            }
        }
        ...
        comparison.setOnClickListener{
            when (comparisonState) {
                true -> {
                    ...
                    Thread{
                        dao.updateComparisonVideocardById(false, id!!)
                    }.start()
                }
                false -> {
                    comparison.setImageResource(R.drawable.comparision_active)
                    Thread{
                        dao.updateComparisonVideocardById(true, id!!)
                    }.start()
                }
            }
        }
        ...
        busket.setOnClickListener{
            when (bucketState) {
                ...
                false -> {
                    ...
                    Thread{
                        dao.updateBucketVideocardById(true, id!!)
                    }.start()
                }
            }
        }
