...
import com.example.pcstore.databinding.MotherboardActivityBinding

class Motherboard :AppCompatActivity() {

    lateinit var binding: MotherboardActivityBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = MotherboardActivityBinding.inflate(layoutInflater)

        var db: DataBase = DataBase.getDatabaseMotherboard(this)

        val dao = db.getDao()

        db.getDao().getAllItemMotherboard().asLiveData().observe(this) {
            val gridMotherboard = findViewById<GridLayout>(R.id.grid_motherboard)

            gridMotherboard.columnCount = 1;

            var i = 0
            it.forEach {
                    createListDynamically(i, gridMotherboard, dao, it)
                i++
            }
        }
    ...
    private fun createListDynamically(row: Int, grid: GridLayout, dao: ItemsDao, it: MotherboardItem) {
        ...
        mark.setOnClickListener{
            when (favoritesState) {
                true -> {
                    ...
                    Thread{
                        dao.updateFavoritesMotherboardById(false, id!!)
                    }.start()
                }
                false -> {
                    mark.setImageResource(R.drawable.bookmark_active)
                    Thread{
                        dao.updateFavoritesMotherboardById(true, id!!)
                    }.start()
                }
            }
        }
        ...
        comparison.setOnClickListener{
            when (comparisonState) {
                true -> {
                    ...
                    Thread{
                        dao.updateComparisonMotherboardById(false, id!!)
                    }.start()
                }
                false -> {
                    comparison.setImageResource(R.drawable.comparision_active)
                    Thread{
                        dao.updateComparisonMotherboardById(true, id!!)
                    }.start()
                }
            }
        }
        ...
        busket.setOnClickListener{
            when (bucketState) {
                ...
                false -> {
                    ...
                    Thread{
                        dao.updateBucketMotherboardById(true, id!!)
                    }.start()
                }
            }
        }
