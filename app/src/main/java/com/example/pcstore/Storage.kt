...
import com.example.pcstore.databinding.SsdActivityBinding

class Storage: AppCompatActivity() {
    lateinit var binding: SsdActivityBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = SsdActivityBinding.inflate(layoutInflater)

        var db: DataBase = DataBase.getDatabaseStorage(this)


        val dao = db.getDao()

        db.getDao().getAllItemStorage().asLiveData().observe(this) {
            val grid = findViewById<GridLayout>(R.id.grid_ssd)
    ...
    private fun createListDynamically(row: Int, grid: GridLayout, dao: ItemsDao, it: StorageItem) {
        ...
        mark.setOnClickListener {
            when (favoritesState) {
                true -> {
                    ...
                    Thread {
                        dao.updateFavoritesStorageById(false, id!!)
                    }.start()
                }
                ...
                false -> {
                    ...
                    Thread {
                        dao.updateFavoritesStorageById(true, id!!)
                    }.start()
                }
            }
        }
        ...
        comparison.setOnClickListener {
            when (comparisonState) {
                true -> {
                    ...
                    Thread {
                        dao.updateComparisonStorageById(false, id!!)
                    }.start()
                }
                ...
                false -> {
                    ...
                    Thread {
                        dao.updateComparisonStorageById(true, id!!)
                    }.start()
                }
            }
        }
        ...
        busket.setOnClickListener {
            when (bucketState) {
                ...
                false -> {
                    ...
                    Thread {
                        dao.updateBucketStorageById(true, id!!)
                    }.start()
                }
            }
        }
